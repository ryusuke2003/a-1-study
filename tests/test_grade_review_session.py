...
class RepositoryFixture:
    def __init__(self, root: Path, count: int, selected: str = "A") -> None:
        ...
        (root / "学習記録/間違えた問題").mkdir(parents=True)
...
class GradeReviewSessionTest(unittest.TestCase):
    ...
    def test_apply_thirty_questions_in_one_atomic_run(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertFalse((root / "学習記録/間違えた問題/2026-09-02.md").exists())
    ...
    def test_wrong_answer_requires_and_writes_all_choice_explanations(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            mistakes = (root / "学習記録/間違えた問題/2026-09-02.md").read_text()
            self.assertIn("### 問題", mistakes)
            self.assertIn("### 模範解答", mistakes)
            for choice in "ABCD":
                self.assertIn(f"- {choice}:", mistakes)
                self.assertIn(f"→ {explanation(choice)}", mistakes)
    ...
    def test_cleanup_removes_moved_comment_without_touching_visible_record(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            mistakes_dir = root / "学習記録/間違えた問題"
            mistakes_dir.mkdir(parents=True)
            path = mistakes_dir / "2026-09-01.md"
            path.write_text(
